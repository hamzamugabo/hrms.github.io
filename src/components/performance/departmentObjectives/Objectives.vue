...
import Create from '@/components/performance/departmentObjectives/Create.vue';
...
const { getDepartmentObjectives } = usePerformanceUtils();
...
const updateObjective = async () => {
    try {
        updateError.value = '';
        loading.value = true;
        const url = `${baseURL}/departmentObjective/update`;
        const formData = {
        ...
        if (data?.status === 200 || data?.status === 201) {
            ...
            await getDepartmentObjectives(objectives);
        } else {
            showError();
...
const deleteObjective = async (id) => {
    ...
    const url = `${baseURL}/departmentObjective/delete/${id}`;
...
const submitForm = async () => {
    ...
    const url = `${baseURL}/departmentObjective/create`;
...
<template>
    <div class="flex justify-content-center">
        <Dialog v-model:visible="visible" header="Edit department objective" :style="{ width: '50%' }">
        ...
        <Dialog v-model:visible="visibleAdd" header="Create department objective" :style="{ width: '50%' }">
    ...
    <div class="card">
        ...
        <DataTable v-model:filters="filters" :value="objectives" paginator showGridlines :rows="10" dataKey="id" filterDisplay="menu" :loading="loading" :globalFilterFields="['name']">
            ...
            <Column :sortable="true" field="organisationObjectiveName" header="Org Objective" style="min-width: 9rem">
                <template #body="{ data }"> {{ data?.organisationObjectiveName }}</template>
            </Column>
            <Column :sortable="true" field="name" header="Dept Objective" style="min-width: 9rem">
