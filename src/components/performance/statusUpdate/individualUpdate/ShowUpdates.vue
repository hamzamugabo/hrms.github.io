...
import CreateStatusUpdate from '@/components/performance/statusUpdate/individualUpdate/Create.vue';
...
const { getIndividualObjectivById } = usePerformanceUtils();
...
const getIndividualUpdates = async () => {
    const url = `${baseURL}/progress_update/individual`;

    try {
        const { data } = await fetchData(url, loading);
        console.log('indi status', data);
        if (Array.isArray(data) && data.length > 0) {
            let allUpdates = await Promise.all(
                data.map(async (element) => {
                    const individual = await getIndividualObjectivById(element?.individualObjectiveId);

                    return {
                        ...element,
                        individualObjectiveName: individual?.name,
                        individualObjectiveDescription: individual?.description
                    };
                })
            );
console.log('all indi status',allUpdates )
        ...
        <div class="grid">
            <div class="col-12 xl:col-7">
                <DataTable v-model:filters="filters" :value="statusUpdates" paginator showGridlines :rows="10" dataKey="id" filterDisplay="menu" :loading="loading" :globalFilterFields="['name']">
                    ...
                    <Column :sortable="true" field=" individualObjectiveName " header="Objective Name" style="min-width: 18rem">
                        <template #body="{ data }"> {{ data.individualObjectiveName }} </template>
                    </Column>
