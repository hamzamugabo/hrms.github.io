...
import Create from '@/components/performance/metrics/internalProcessMetric/Create.vue';
...
const updateMetric = async () => {
    ...
    try {
        loading.value = true;
        const url = `${baseAUTHURL}/performance_metric/internal_process/update`;
        const formData = {
...
const handleConfirmDelete = async (data) => {
    const url = `${baseAUTHURL}/performance_metric/internal_process/delete/${data?.id}`;
...
<template>
    <div class="flex justify-content-center">
        ...
        <Dialog v-model:visible="addModalVisibility" header="Add Internal Process Metric">
            <Create />
        </Dialog>
    </div>
    ...
    <div class="card">
        <div class="p-fluid formgrid grid flex-grow">
            <div class="field col-12 md:col-12">
                <AddButton :label="'Internal Process Metric'" @handleAdd="handleAdd" />
            </div>
        </div>
