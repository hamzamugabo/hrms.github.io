...
import Create from '@/components/performance/metrics/supervisorBehavioralMetric/Create.vue';
...
const { getSupervisorBehavioralMetric } = usePerformanceUtils();
...
const updateMetric = async () => {
    ...
    try {
        loading.value = true;
        const url = `${baseURL}/performance_metric/supervisor_behavioral/update/supervisor`;
        const formData = {
        ...
        if (data?.status === 200 || data?.status === 201) {
            ...
            await getSupervisorBehavioralMetric(performanceMetrics);
        } else {
            showError();
...
const handleConfirmDelete = async (data) => {
    const url = `${baseURL}/performance_metric/supervisor_behavioral/delete/${data?.id}`;
...
<template>
    <div class=" flex justify-content-center">
        <Dialog v-model:visible="visible" header="Edit Supervisor Behavioral Metric" :style="{ width: '30rem' }">
        ...
        <Dialog v-model:visible="addModalVisibility" header="Add Supervisor Behavioral Metric">
            <Create />
        </Dialog>
    </div>
    ...
    <div class="card">
        <div class="p-fluid formgrid grid flex-grow">
            <div class="field col-12 md:col-12">
                <AddButton :label="'Supervisor Behavioral Metric'" @handleAdd="handleAdd" />
            </div>
        </div>
        ...
        <DataTable v-model:filters="filters" :value="performanceMetrics" paginator showGridlines :rows="10" dataKey="id" filterDisplay="menu" :loading="loading" :globalFilterFields="['metricCategory', 'metricDescription']">
            ...
            <template #empty> No Supervisor Behavioral Metrics found. </template>
            <template #loading> Loading Supervisor Behavioral Metric data. Please wait. </template>
