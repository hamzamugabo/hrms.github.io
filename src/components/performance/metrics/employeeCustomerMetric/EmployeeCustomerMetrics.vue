...
import Create from '@/components/performance/metrics/employeeCustomerMetric/Create.vue';
...
const { getEmployeeCustomerMetric } = usePerformanceUtils();
...
const updateMetric = async () => {
    ...
    try {
        loading.value = true;
        const url = `${baseURL}/performance_metric/customer/update`;
        const formData = {
        ...
        if (data?.status === 200 || data?.status === 201) {
            ...
            await getEmployeeCustomerMetric(performanceMetrics);
        } else {
            showError();
...
const handleConfirmDelete = async (data) => {
    const url = `${baseURL}/performance_metric/customer/delete/${data?.id}`;
...
<template>
    <div class="flex justify-content-center">
        <Dialog v-model:visible="visible" header="Edit Employee Customer Metric" :style="{ width: '30rem' }">
        ...
        <Dialog v-model:visible="addModalVisibility" header="Add Employee Customer Metric">
            <Create />
        </Dialog>
    </div>
    ...
    <div class="card">
        <div class="p-fluid formgrid grid flex-grow">
            <div class="field col-12 md:col-12">
                <AddButton :label="'Customer Metric'" @handleAdd="handleAdd" />
            </div>
        </div>
