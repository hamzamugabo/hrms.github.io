...
import Create from '@/components/performance/metrics/financialMetric/Create.vue';
...
const { getFinancialMetric } = usePerformanceUtils();
...
const updateMetric = async () => {
    ...
    try {
        loading.value = true;
        const url = `${baseURL}/performance_metric/financial/update`;
        const formData = {
        ...
        if (data?.status === 200 || data?.status === 201) {
            ...
            await getFinancialMetric(performanceMetrics);
        } else {
            showError();
...
const handleConfirmDelete = async (data) => {
    const url = `${baseURL}/performance_metric/financial/delete/${data?.id}`;
...
<template>
    <div class="flex justify-content-center">
        <Dialog v-model:visible="visible" header="Edit Financial Metric" :style="{ width: '30rem' }">
        ...
        <Dialog v-model:visible="addModalVisibility" header="Add Financial Metric">
            <Create />
        </Dialog>
    </div>
    ...
    <div class="card">
        <div class="p-fluid formgrid grid flex-grow">
            <div class="field col-12 md:col-12">
                <AddButton :label="'Financial Metric'" @handleAdd="handleAdd" />
            </div>
        </div>
        ...
        <DataTable v-model:filters="filters" :value="performanceMetrics" paginator showGridlines :rows="10" dataKey="id" filterDisplay="menu" :loading="loading" :globalFilterFields="['metricCategory', 'metricDescription']">
            ...
            <template #empty> No Financial Metrics found. </template>
            <template #loading> Loading Financial Metric data. Please wait. </template>
