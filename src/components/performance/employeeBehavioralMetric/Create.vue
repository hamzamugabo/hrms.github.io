<script setup>
import { postData } from '@/components/reuseables/FetchPostData';
import SpinnerVue from '@/components/reuseables/Spinner.vue';
import { onMounted, ref } from 'vue';
import { baseURL } from '@/components/reuseables/FetchPostData';
import { usePerformanceUtils } from '@/components/performance/performanceUtils';
import { useToastPopup } from '@/components/reuseables/useToast.js';

const { showError, showSuccess } = useToastPopup();
const { getPerformanceMetrics } = usePerformanceUtils();

const employeeCustomerCare = ref('');
const employeeInnovation = ref('');
const employeeIntegrity = ref('');
const performanceMetricId = ref('');
const employeeProfessionalism = ref('');
const employeeQualityOfService = ref('');
const employeeRiskManagement = ref('');
const employeeTeamwork = ref('');
const loading = ref(false);
const success = ref(false);
const autoFilteredPerformanceMetrics = ref([]);
const allPerformanceMetrics = ref([]);

const serverError = ref('');
const successMessage = ref('');

const submitForm = async () => {
    loading.value = true;
      serverError.value = '';
    const formData = {
        employeeCustomerCare: employeeCustomerCare.value,
        performanceMetricId: performanceMetricId?.value?.id,
        employeeInnovation: employeeInnovation.value,
        employeeIntegrity: employeeIntegrity.value,
        employeeProfessionalism: employeeProfessionalism.value,
        employeeQualityOfService: employeeQualityOfService.value,
        employeeRiskManagement: employeeRiskManagement.value,
        employeeTeamwork: employeeTeamwork.value
    };

    const url = `${baseURL}/performance_metric/employee_behavioral/create`;

    try {
        const data = await postData(url, formData);
        if (data?.status === 200 || data?.status === 201) {
            successMessage.value = data?.message;
            // success.value = true;
            showSuccess( data?.message);
            loading.value = false;
        } else {
            showError()
            serverError.value = data?.error || data?.message;
            loading.value = false;
        }
    } catch (error) {
            showError()

        loading.value = false;
        console.log(error?.error);
    }
};

const searchPerformanceMetric = (event) => {
    if (!event.query.trim().length) {
        autoFilteredPerformanceMetrics.value = allPerformanceMetrics?.value?.length > 0 ? [...allPerformanceMetrics.value] : [];
    } else {
        autoFilteredPerformanceMetrics.value = allPerformanceMetrics.value.filter((metric) => {
            const metricCategory = metric.metricCategory.toLowerCase().startsWith(event.query.toLowerCase());
            const metricDescription = metric.metricDescription.toLowerCase().startsWith(event.query.toLowerCase());

            return metricCategory || metricDescription;
        });
    }
};
onMounted(async () => {
    await getPerformanceMetrics(allPerformanceMetrics);
});
</script>
<template>
    <div class="form-container">
        <form @submit.prevent="submitForm" class="form-content">
            <div class="grid">
                <!-- <i class="pi pi-spin pi-spinner" style="font-size: 2rem" v-if="loading"></i> -->

                <div class="col-12">
                    <div class="card">
                        <Message v-if="success" severity="success">{{ successMessage }}</Message>

                        <Message v-if="serverError" severity="error">{{ serverError }}</Message>
                        <h5>Create Job Description</h5>
                        <div class="p-fluid formgrid grid">
                            <div class="field col-12 md:col-12">
                                <label for="performanceMetric">Performance Metric</label>
                                <AutoComplete
                                    placeholder="Search"
                                    id="performanceMetric"
                                    :dropdown="true"
                                    :multiple="false"
                                    v-model="performanceMetricId"
                                    :suggestions="autoFilteredPerformanceMetrics"
                                    @complete="searchPerformanceMetric($event)"
                                    field="metricCategory"
                                >
                                    <template #option="slotProps">
                                        <div class="job-info">
                                            <div class="flex align-options-center">
                                                <div class="job-detail">
                                                    <div>metricCategory: {{ slotProps?.option?.metricCategory }}</div>
                                                    <div>metricDescription: {{ slotProps?.option?.metricDescription }}</div>
                                                </div>
                                            </div>
                                        </div>
                                    </template>
                                </AutoComplete>
                            </div>
                            <div class="field col-12 md:col-12">
                                <label for="employeeCustomerCare">employee Customer Care</label>
                                <InputText required id="employeeCustomerCare" type="text" v-model="employeeCustomerCare" />
                            </div>
                            <div class="field col-12 md:col-12">
                                <label for="employeeInnovation">employee Innovation</label>
                                <InputText required id="employeeInnovation" type="text" v-model="employeeInnovation" />
                            </div>

                            <div class="field col-12 md:col-12">
                                <label for="employeeIntegrity">employee Integrity</label>
                                <InputText id="employeeIntegrity" type="text" v-model="employeeIntegrity" />
                            </div>
                            <div class="field col-12 md:col-12">
                                <label for="employeeProfessionalism">employee Professionalism</label>
                                <InputText id="employeeProfessionalism" type="text" v-model="employeeProfessionalism" />
                            </div>
                            <div class="field col-12 md:col-12">
                                <label for="employeeQualityOfService">employee Quality Of Service</label>
                                <InputText id="employeeQualityOfService" type="text" v-model="employeeQualityOfService" />
                            </div>
                            <div class="field col-12 md:col-12">
                                <label for="employeeRiskManagement">employee Risk Management</label>
                                <InputText id="employeeRiskManagement" type="text" v-model="employeeRiskManagement" />
                            </div>
                            <div class="field col-12 md:col-12">
                                <label for="employeeTeamwork">employee Team work</label>
                                <InputText id="employeeTeamwork" type="text" v-model="employeeTeamwork" />
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div style="display: flex; justify-content: center">
                <SpinnerVue :loading="loading" size="3rem" />
            </div>

            <div v-if="!loading" class="mb-10" style="margin: 50px 0px">
                <button style="background-color: #db0000; color: #ffffff; padding: 0.5rem 1rem; border-radius: 0.5rem; padding-bottom: 40px; cursor: pointer" class="block mx-auto border-none pb-3">Submit</button>
            </div>
        </form>
    </div>
</template>

<style scoped>
.bg-light-grey {
    background-color: #f0f0f0;
    cursor: pointer;
}
</style>
